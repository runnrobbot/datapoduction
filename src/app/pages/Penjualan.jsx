...
export default function Penjualan() {
  ...
  async function loadAll() {
    setLoading(true);
    try {
      const [penjualan, barang] = await Promise.all([getAllPenjualan(), getAllBarang()]);
      setList(penjualan);
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          ...
          {filterTipe !== 'all' && <p className="text-xs text-slate-400">filter: {filterTipe}</p>}
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <p className="text-xs text-slate-500 uppercase tracking-wider">Total Revenue</p>
          <p className="text-2xl font-bold text-emerald-600 mt-1">{formatCurrency(summary.totalRevenue)}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4 col-span-2 sm:col-span-1">
          <p className="text-xs text-slate-500 uppercase tracking-wider">Jumlah Transaksi</p>
          <p className="text-2xl font-bold text-slate-800 mt-1">{filtered.length.toLocaleString()}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 sm:p-5">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <label className="flex items-center gap-1.5 px-3 py-2 text-sm rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 cursor-pointer">
